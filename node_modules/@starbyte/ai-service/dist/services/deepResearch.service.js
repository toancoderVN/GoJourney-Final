...
export class DeepResearchService {
    ai;
    model = 'gemini-2.5-flash'; // Same as WebSearch
    baseSystemPrompt = `Bạn là GoJourney - chuyên gia nghiên cứu du lịch chuyên sâu.
...
${memoryContext.formattedText}`;
            ...
            for await (const chunk of responseStream) {
                ...
                if (candidate?.groundingMetadata) {
                    ...
                    if (metadata.webSearchQueries && metadata.webSearchQueries.length > 0) {
                        const newQueries = metadata.webSearchQueries.filter((q) => !searchQueries.includes(q));
     ...
     */
    async storeResearchMemory(userId, sessionId, query, content) {
        await memoryService.extractAndStoreFromConversation(userId, sessionId || 'unknown', query, content, 'deep_research');
        console.log('💾 [DeepResearch] Research stored as memory');
    }
}
//# sourceMappingURL=deepResearch.service.js.map