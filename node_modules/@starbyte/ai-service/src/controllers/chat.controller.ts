...
const getAzureAIService = (): AzureAIService => {
  if (!azureAIService) {
    ...
    console.log('[ChatController] AI token configured:', !!process.env.GITHUB_TOKEN, 'tokenPrefix:', process.env.GITHUB_TOKEN ? process.env.GITHUB_TOKEN.slice(0, 8) + '...' : '(none)');
  }
  return azureAIService;
};

const getBookingOrchestrator = (): BookingOrchestrator => {
  if (!bookingOrchestratorInstance) {
    bookingOrchestratorInstance = new BookingOrchestrator(getAzureAIService());
  }
...
export const sendMessage = async (req: Request, res: Response) => {
  try {
    ...
    const hasActiveBooking = getBookingOrchestrator().hasActiveSession(effectiveSessionId);
    ...
    if (bookingRequest || hasActiveBooking) {
      ...
      response = await getBookingOrchestrator().handleBookingRequest(
        effectiveSessionId,
      ...
      response = await getAzureAIService().generateChatResponse(
        messages,
        language,
...
export const executeQuickAction = async (req: Request, res: Response) => {
  try {
    ...
    const response = await getAzureAIService().executeQuickAction(actionId, data, language);
