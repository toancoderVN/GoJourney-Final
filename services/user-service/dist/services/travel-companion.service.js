...
let TravelCompanionService = class TravelCompanionService {
    ...
    notificationService;
    notificationGateway;
    constructor(companionRepository, invitationRepository, userRepository, userCodeService, notificationService, notificationGateway) {
    ...
    async getUserCompanions(userId) {
        ...
        for (const companion of updatedCompanions) {
            if (companion.companion) {
                ...
                companion.currentStatus = isOnline ? travel_companion_entity_1.CurrentStatus.ONLINE : travel_companion_entity_1.CurrentStatus.OFFLINE;
    ...
    async getCompanionStats(userId) {
        const companions = await this.getUserCompanions(userId);
        const invitations = await this.getPendingInvitations(userId);
        const primaryTravelers = companions.filter(c => c.role === travel_companion_entity_1.UserRole.PRIMARY).length;
        const companionUsers = companions.filter(c => c.role === travel_companion_entity_1.UserRole.COMPANION).length;
        const connected = companions.filter(c => c.status === travel_companion_entity_1.ConnectionStatus.CONNECTED).length;
        const pending = companions.filter(c => c.status === travel_companion_entity_1.ConnectionStatus.PENDING).length + invitations.length;
    ...
    async createInvitation(senderId, createDto) {
        // Validate sender exists
        const sender = await this.userRepository.findOne({ where: { id: senderId } });
        if (!sender) {
            throw new common_1.NotFoundException('Người gửi lời mời không tồn tại');
        }
        const invitation = new companion_invitation_entity_1.CompanionInvitation();
        ...
        if (createDto.type === companion_invitation_entity_1.InvitationType.EMAIL) {
            if (!createDto.recipientEmail) {
                throw new common_1.BadRequestException('Email người nhận là bắt buộc cho loại lời mời này');
            }
        ...
        else if (createDto.type === companion_invitation_entity_1.InvitationType.USER_ID) {
            if (!createDto.recipientId) {
                throw new common_1.BadRequestException('ID người nhận là bắt buộc cho loại lời mời này');
            }
            ...
            if (existingConnection) {
                throw new common_1.ConflictException('Đã có kết nối với người dùng này');
            }
            invitation.recipientId = createDto.recipientId;
        }
        else if (createDto.type === companion_invitation_entity_1.InvitationType.LINK) {
            invitation.inviteCode = this.generateInviteCode();
            invitation.expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days
        }
        return await this.invitationRepository.save(invitation);
    }
    async connectByUserId(userId, connectDto) {
        ...
        if (!recipient) {
            throw new common_1.NotFoundException('Không tìm thấy người dùng với ID này');
        }
        return await this.createInvitation(userId, {
            type: companion_invitation_entity_1.InvitationType.USER_ID,
            recipientId: recipient.id,
            message: connectDto.message
        });
    }
    async acceptInvitation(invitationId, userId, acceptDto) {
        ...
        if (invitation.status !== companion_invitation_entity_1.InvitationStatus.PENDING) {
        ...
        const companion1 = new travel_companion_entity_1.TravelCompanion();
        companion1.user = { id: invitation.senderId };
        companion1.companion = { id: userId };
        companion1.relationship = acceptDto.relationship;
        companion1.status = travel_companion_entity_1.ConnectionStatus.CONNECTED;
        companion1.role = travel_companion_entity_1.UserRole.COMPANION;
        const companion2 = new travel_companion_entity_1.TravelCompanion();
        companion2.user = { id: userId };
        companion2.companion = { id: invitation.senderId };
        companion2.relationship = acceptDto.relationship;
        companion2.status = travel_companion_entity_1.ConnectionStatus.CONNECTED;
        companion2.role = travel_companion_entity_1.UserRole.COMPANION;
        // Save both relationships
        await this.companionRepository.save([companion1, companion2]);
        // Update invitation status
        invitation.status = companion_invitation_entity_1.InvitationStatus.ACCEPTED;
    ...
    async declineInvitation(invitationId, userId) {
        ...
        invitation.status = companion_invitation_entity_1.InvitationStatus.DECLINED;
        invitation.respondedAt = new Date();
        await this.invitationRepository.save(invitation);
    }
    async getPendingInvitations(userId) {
        ...
        const invitations = await this.invitationRepository.find({
            where: [
                { senderId: userId, status: companion_invitation_entity_1.InvitationStatus.PENDING },
                { recipientId: userId, status: companion_invitation_entity_1.InvitationStatus.PENDING }
            ],
            relations: ['sender', 'recipient'],
            order: { createdAt: 'DESC' }
        });
    ...
    async updateCompanion(userId, companionId, updateDto) {
        const companion = await this.companionRepository.findOne({
            where: { user: { id: userId }, companion: { id: companionId } }
        });
        if (!companion) {
            throw new common_1.NotFoundException('Người đồng hành không tồn tại');
        }
    ...
    async updateTravelPreferences(userId, companionId, preferencesDto) {
        const companion = await this.companionRepository.findOne({
            where: { user: { id: userId }, companion: { id: companionId } }
        });
        if (!companion) {
    ...
    async blockCompanion(userId, companionId) {
        const companion = await this.companionRepository.findOne({
            where: { user: { id: userId }, companion: { id: companionId } }
        });
        if (!companion) {
        ...
        companion.status = travel_companion_entity_1.ConnectionStatus.BLOCKED;
        await this.companionRepository.save(companion);
    }
    async unblockCompanion(userId, companionId) {
        const companion = await this.companionRepository.findOne({
            where: { user: { id: userId }, companion: { id: companionId } }
        });
        if (!companion) {
        ...
        companion.status = travel_companion_entity_1.ConnectionStatus.CONNECTED;
        await this.companionRepository.save(companion);
    }
    async getInvitationByCode(inviteCode) {
        ...
        if (!invitation) {
            throw new common_1.NotFoundException('Mã mời không tồn tại');
        }
        if (invitation.expiresAt && invitation.expiresAt < new Date()) {
            throw new common_1.BadRequestException('Mã mời đã hết hạn');
        }
        return invitation;
    }
    async incrementTripCount(userId, companionId) {
        const companion = await this.companionRepository.findOne({
            where: { user: { id: userId }, companion: { id: companionId } }
        });
        if (companion) {
    ...
    async connectByUserCode(userId, userCode, relationship, message) {
        const targetUser = await this.userCodeService.findUserByCode(userCode);
        if (!targetUser) {
            throw new common_1.NotFoundException('Mã người dùng không tồn tại hoặc đã hết hạn');
        }
        if (targetUser.id === userId) {
            throw new common_1.BadRequestException('Không thể kết nối với chính mình');
        }
        ...
        if (existingConnection) {
            throw new common_1.ConflictException('Đã kết nối với người dùng này');
        }
        // Check for pending invitation
        const existingInvitation = await this.invitationRepository.findOne({
            where: [
                { senderId: userId, recipientId: targetUser.id, status: companion_invitation_entity_1.InvitationStatus.PENDING },
                { senderId: targetUser.id, recipientId: userId, status: companion_invitation_entity_1.InvitationStatus.PENDING }
            ]
        });
        if (existingInvitation) {
            throw new common_1.ConflictException('Đã có lời mời đang chờ xử lý');
        }
        // Create invitation
        const invitation = this.invitationRepository.create({
            ...
            type: companion_invitation_entity_1.InvitationType.USER_ID,
            status: companion_invitation_entity_1.InvitationStatus.PENDING,
        ...
        await this.notificationService.createNotification(targetUser.id, notification_entity_1.NotificationType.COMPANION_INVITATION, 'Lời mời kết nối mới', `${sender?.firstName} ${sender?.lastName} muốn kết nối làm người đồng hành du lịch`, { invitationId: savedInvitation.id, senderName: `${sender?.firstName} ${sender?.lastName}` }, savedInvitation.id);
    ...
    async generateInviteLink(userId, relationship, message, tripId) {
        const inviteCode = this.generateInviteCode();
        const invitation = this.invitationRepository.create({
            senderId: userId,
            type: companion_invitation_entity_1.InvitationType.LINK,
            status: companion_invitation_entity_1.InvitationStatus.PENDING,
    ...
    async acceptInviteByCode(userId, inviteCode, relationship) {
        const invitation = await this.invitationRepository.findOne({
            where: { inviteCode, status: companion_invitation_entity_1.InvitationStatus.PENDING },
            relations: ['sender']
        });
        if (!invitation) {
            throw new common_1.NotFoundException('Lời mời không tồn tại hoặc đã hết hạn');
        }
        if (invitation.expiresAt && invitation.expiresAt < new Date()) {
            throw new common_1.BadRequestException('Lời mời đã hết hạn');
        }
        if (invitation.senderId === userId) {
            throw new common_1.BadRequestException('Không thể chấp nhận lời mời của chính mình');
        }
        // Check if already connected
        const existingConnection = await this.companionRepository.findOne({
            where: [
        ...
        const companion1 = this.companionRepository.create({
            user: { id: userId },
            companion: { id: invitation.senderId },
            relationship,
            status: travel_companion_entity_1.ConnectionStatus.CONNECTED,
            role: travel_companion_entity_1.UserRole.COMPANION,
            connectionDate: new Date(),
            sharedTrips: 0
        });
        const companion2 = this.companionRepository.create({
            user: { id: invitation.senderId },
            companion: { id: userId },
            relationship,
            status: travel_companion_entity_1.ConnectionStatus.CONNECTED,
            role: travel_companion_entity_1.UserRole.COMPANION,
        ...
        await this.notificationService.createNotification(invitation.senderId, notification_entity_1.NotificationType.COMPANION_ACCEPTED, 'Lời mời được chấp nhận', `${recipient?.firstName} ${recipient?.lastName} đã chấp nhận lời mời kết nối`, { companionId: userId, companionName: `${recipient?.firstName} ${recipient?.lastName}` }, companion1.id);
    ...
    async inviteToTrip(userId, companionId, tripId, message) {
        // Check if they are connected
        const connection = await this.companionRepository.findOne({
            where: { user: { id: userId }, companion: { id: companionId }, status: travel_companion_entity_1.ConnectionStatus.CONNECTED }
        });
        if (!connection) {
            throw new common_1.BadRequestException('Chưa kết nối với người dùng này');
        }
        ...
        if (existingInvitation) {
            throw new common_1.ConflictException('Đã gửi lời mời cho chuyến đi này');
        }
        // Create trip invitation
        const invitation = this.invitationRepository.create({
            senderId: userId,
            recipientId: companionId,
            type: companion_invitation_entity_1.InvitationType.SYSTEM,
            status: companion_invitation_entity_1.InvitationStatus.PENDING,
        ...
        // Create notification
        const sender = await this.userRepository.findOne({ where: { id: userId } });
        await this.notificationService.createNotification(companionId, notification_entity_1.NotificationType.TRIP_INVITATION, 'Lời mời tham gia chuyến đi', `${sender?.firstName} ${sender?.lastName} mời bạn tham gia chuyến đi`, {
            invitationId: savedInvitation.id,
            tripId,
    ...
    async acceptTripInvitation(invitationId, userId) {
        const invitation = await this.invitationRepository.findOne({
            where: {
                id: invitationId,
                recipientId: userId,
        ...
        if (!invitation.tripId) {
            throw new common_1.BadRequestException('Lời mời không hợp lệ');
        }
        // Update invitation
        invitation.status = companion_invitation_entity_1.InvitationStatus.ACCEPTED;
        invitation.respondedAt = new Date();
        ...
        // Create notification for sender
        const recipient = await this.userRepository.findOne({ where: { id: userId } });
        await this.notificationService.createNotification(invitation.senderId, notification_entity_1.NotificationType.TRIP_ACCEPTED, 'Lời mời chuyến đi được chấp nhận', `${recipient?.firstName} ${recipient?.lastName} đã chấp nhận tham gia chuyến đi`, {
    ...
    async removeCompanion(userId, companionId) {
        ...
        if (!companionRelation) {
            throw new common_1.NotFoundException('Không tìm thấy mối quan hệ đồng hành');
        }
        ...
        if (companionRelation.userId !== userId && companionRelation.companionId !== userId) {
            throw new common_1.BadRequestException('Bạn không có quyền xóa mối quan hệ này');
        }
        ...
        if (relationshipsToRemove.length === 0) {
            throw new common_1.NotFoundException('Không tìm thấy mối quan hệ để xóa');
        }
        // Get user names for notifications
...
exports.TravelCompanionService = TravelCompanionService = __decorate([
    ...
    __param(1, (0, typeorm_1.InjectRepository)(companion_invitation_entity_1.CompanionInvitation)),
