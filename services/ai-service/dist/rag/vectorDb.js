 ...
 */
export async function addDocumentToChroma(id, text, metadata, collectionName = DEFAULT_COLLECTION) {
    try {
 ...
 * @param collectionName - Optional collection name (defaults to booking_context)
 */
export async function searchChroma(query, topK = 5, filter, collectionName = DEFAULT_COLLECTION) {
    try {
        const collectionId = await getOrCreateCollection(collectionName);
        ...
        const res = await axios.post(`${CHROMA_API}/api/v1/collections/${collectionId}/query`, payload);
 ...
 * @param collectionName - Optional collection name (defaults to booking_context)
 */
export async function deleteCollection(collectionName = DEFAULT_COLLECTION) {
    try {
        const collectionId = await getOrCreateCollection(collectionName);
 ...
 * @param collectionName - Optional collection name (defaults to booking_context)
 */
export async function getCollectionInfo(collectionName = DEFAULT_COLLECTION) {
    try {
        const collectionId = await getOrCreateCollection(collectionName);
